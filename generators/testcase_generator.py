...
def parse_constraints(description_text):
    """
...
def generate_edge_case_value(constraints, case_type='max'):
...
def generate_array_testcase(constraints, case_type='max'):
    ...
    if case_type == 'all_same':
        val = generate_edge_case_value(constraints, 'max')
        return [val] * length
    elif case_type == 'all_negative':
        if constraints['allows_negative']:
            val = generate_edge_case_value(constraints, 'negative')
            return [val] * length
        return [generate_edge_case_value(constraints, 'min')] * length
        ...
        return [generate_edge_case_value(constraints, 'random') for _ in range(length)]


def generate_string_testcase(constraints, case_type='max'):
...
def generate_additional_testcases(data, description_text, original_count):
    """
    ...
    """
    constraints = parse_constraints(description_text)
    ...
    for i in range(needed):
        ...
        if param_count <= 1:
            # Single parameter
            if is_array_problem:
                case = generate_array_testcase(constraints, case_type)
            elif is_string_problem:
                case = generate_string_testcase(constraints, case_type)
            else:
                case = generate_edge_case_value(constraints, case_type)
            ...
            for param_idx in range(param_count):
                if param_idx == 0:  # Usually the main array/string
                    if is_array_problem:
                        case_group.append(generate_array_testcase(constraints, case_type))
                    elif is_string_problem:
                        case_group.append(generate_string_testcase(constraints, case_type))
                    else:
                        case_group.append(generate_edge_case_value(constraints, case_type))
                else:  # Secondary parameters (like target)
                    case_group.append(generate_edge_case_value(constraints, 'random'))
            additional_cases.extend(case_group)
    
    return additional_cases


def format_testcases_for_submission(all_cases, language='python'):
...
def format_testcases_for_leetcode(all_cases):
...
def json_repr(obj):
    ...
    if isinstance(obj, list):
        return '[' + ', '.join(json_repr(item) for item in obj) + ']'
...
def generate_testcase_comment(data, description_text):
    ...
    additional = generate_additional_testcases(data, description_text, original_count)
    ...
    submission_format = format_testcases_for_submission(all_cases, language)
    
    # Format for LeetCode
    leetcode_format = format_testcases_for_leetcode(all_cases)
